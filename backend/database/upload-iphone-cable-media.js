...
const imagePaths = [
  {
    path: 'C:/Users/acer/.gemini/antigravity/brain/aed2b88d-8f79-4dad-9936-d9125b6cf884/.user_uploaded/media_1788769252469.jpg',
    name: 'iphone-lightning-cable-front-packaging.jpg',
    role: 'Primary Hero Shot (Front packaging of Power Boost USB-A to Lightning Cable)',
    sort_order: 0,
    is_primary: true
  },
  {
    path: 'C:/Users/acer/.gemini/antigravity/brain/aed2b88d-8f79-4dad-9936-d9125b6cf884/.user_uploaded/media_1788769252490.jpg',
    name: 'iphone-lightning-cable-box-in-hand.jpg',
    role: 'Secondary Detail Shot (Packaging held in hand showing specifications)',
    sort_order: 1,
    is_primary: false
  }
];
...
async function uploadIphoneCableMedia() {
  console.log('=== UPLOADING IPHONE CABLE CHARGER PRODUCT MEDIA ===\n');

  // 1. Get Admin Token
  const token = await getAdminToken();
  console.log('[Auth] Admin token acquired.');

  // 2. Locate the "Original iPhone Cable Charger" product
  const productsRes = await fetch(`${BASE_URL}/api/products?all=true`);
  const productsData = await productsRes.json();
  const product = productsData.data.find(p => p.name_en === "Original iPhone Cable Charger");

  if (!product) {
    throw new Error('Could not find product: "Original iPhone Cable Charger"');
  }
...
uploadIphoneCableMedia().catch(err => {
