...
const imagePaths = [
  {
    path: 'C:/Users/acer/.gemini/antigravity/brain/aed2b88d-8f79-4dad-9936-d9125b6cf884/.user_uploaded/media_1788773533313.jpg',
    name: 'fmax-powerbank-hero-lifestyle.jpg',
    role: 'Primary Hero Shot (Retail box standing next to power bank with 100% digital display and USB cable on desk)',
    sort_order: 0,
    is_primary: true
  },
  {
    path: 'C:/Users/acer/.gemini/antigravity/brain/aed2b88d-8f79-4dad-9936-d9125b6cf884/.user_uploaded/media_1788773533300.jpg',
    name: 'fmax-powerbank-360-infographic.jpg',
    role: 'Secondary Feature Shot (360-degree views, port diagrams, built-in cable details, and specs infographic)',
    sort_order: 1,
    is_primary: false
  }
];
...
async function uploadPowerBankMedia() {
  console.log('=== UPLOADING F-MAX POWER BANK PRODUCT MEDIA ===\n');

  // 1. Get Admin Token
  const token = await getAdminToken();
  console.log('[Auth] Admin token acquired.');

  // 2. Locate the "F-max TD-301 30,000mAh Power Bank" product
  const productsRes = await fetch(`${BASE_URL}/api/products?all=true`);
  const productsData = await productsRes.json();
  const product = productsData.data.find(p => p.name_en === 'F-max TD-301 30,000mAh Power Bank');

  if (!product) {
    throw new Error('Could not find product: "F-max TD-301 30,000mAh Power Bank"');
  }
  ...
  // 3. Check if media already exists for this product to prevent duplicates
...
uploadPowerBankMedia().catch(err => {
