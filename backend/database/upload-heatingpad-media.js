...
const imagePaths = [
  {
    path: 'C:/Users/acer/.gemini/antigravity/brain/aed2b88d-8f79-4dad-9936-d9125b6cf884/.user_uploaded/media_1788766726286.jpg',
    name: 'womens-period-cramp-relief-wearable-device.jpg',
    role: 'Primary Hero Shot (Wearable pad with adjustable strap & LED display)',
    sort_order: 0,
    is_primary: true
  },
  {
    path: 'C:/Users/acer/.gemini/antigravity/brain/aed2b88d-8f79-4dad-9936-d9125b6cf884/.user_uploaded/media_1788766726210.jpg',
    name: 'womens-period-cramp-relief-color-variants.jpg',
    role: 'Color Variants & Units Showcase (Pink and White units in open boxes)',
    sort_order: 1,
    is_primary: false
  },
  {
    path: 'C:/Users/acer/.gemini/antigravity/brain/aed2b88d-8f79-4dad-9936-d9125b6cf884/.user_uploaded/media_1788766726243.jpg',
    name: 'womens-period-cramp-relief-gift-packaging.jpg',
    role: 'Gift Packaging (Pink box with gold ribbon detail)',
    sort_order: 2,
    is_primary: false
  }
];
...
async function uploadHeatingPadMedia() {
  console.log('=== UPLOADING HEATING PAD PRODUCT MEDIA ===\n');

  // 1. Get Admin Token
  const token = await getAdminToken();
  console.log('[Auth] Admin token acquired.');

  // 2. Locate the "Original Women's Period Cramp Relief" product
  const productsRes = await fetch(`${BASE_URL}/api/products?all=true`);
  const productsData = await productsRes.json();
  const product = productsData.data.find(p => p.name_en === "Original Women's Period Cramp Relief");

  if (!product) {
    throw new Error('Could not find product: "Original Women\'s Period Cramp Relief"');
  }
...
uploadHeatingPadMedia().catch(err => {
